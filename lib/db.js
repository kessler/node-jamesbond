...
var publicApi = module.exports = {
	...
	*/
	putApp: function(app, cb) {
		...
		fs.writeFile(path.join(config.db, app.name + '.app'), JSON.stringify(app), 'utf8', cb)
	},
	...
	*/
	getApp: function(name, cb) {
		fs.readFile(path.join(config.db, name + '.app'), 'utf8', function(err, data) {
	...
	deleteApp: function(name, cb) {
		fs.unlink(path.join(config.db, name + '.app'), cb)
	},
